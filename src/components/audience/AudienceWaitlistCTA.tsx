...
export function AudienceWaitlistCTA({
  source,
  idPrefix,
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    setNotice(null);
    const result = await submitWaitlist({ email: cleaned, source });
    setSubmitting(false);
  ...
  const policyId = `${idPrefix}-waitlist-policy`;
  ...
  return (
    <section
      ...
      aria-labelledby={`${idPrefix}-waitlist-heading`}
    >
      <div className="mx-auto max-w-content px-4 sm:px-6 md:px-8">
        <SectionReveal className="relative overflow-hidden rounded-brand-lg border border-gold/25 bg-navy-deep/70 px-6 py-10 backdrop-blur-sm md:px-10 md:py-12">
          <h2 id={`${idPrefix}-waitlist-heading`} className="text-center font-heading text-3xl text-gold md:text-4xl">
            {heading}
          </h2>
          <p className="mx-auto mt-5 max-w-3xl text-center text-cream/85">{body}</p>
          ...
          <form onSubmit={handleSubmit} className="mx-auto mt-8 max-w-xl" noValidate>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <input
                ...
                aria-describedby={error ? errorId : policyId}
            ...
            <p id={policyId} className="mt-3 text-xs text-cream/65">
