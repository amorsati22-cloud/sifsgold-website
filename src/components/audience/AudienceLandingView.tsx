...
export function AudienceLandingView({ config }: { config: AudienceLandingConfig }) {
  ...
  return (
    <div className="-mx-4 flex min-w-0 flex-1 flex-col sm:-mx-6 md:-mx-8">
      ...
      <section className="border-b border-gold/10 bg-navy py-16 md:py-20" aria-labelledby={`${config.slug}-features-heading`}>
        <div className="mx-auto max-w-content px-4 sm:px-6 md:px-8">
          <SectionReveal>
            ...
            <p className="mt-3 max-w-2xl text-sm text-cream/80">
              Tools shaped with Sif&apos;s Advocates so workflows match how {config.eyebrow.toLowerCase()} actually operate
              inside The Gold Collective.
            </p>
      ...
      <section className="border-b border-gold/10 bg-navy-deep/40 py-14 md:py-16" aria-labelledby={`${config.slug}-testimonial-heading`}>
        <div className="mx-auto max-w-content px-4 sm:px-6 md:px-8">
          <SectionReveal>
      ...
      <AudienceWaitlistCTA source={config.source} idPrefix={config.slug} />
      ...
      >
        <div className="mx-auto flex max-w-content flex-col gap-3 px-4 sm:flex-row sm:items-center sm:justify-center sm:gap-8 sm:px-6 md:px-8">
          <Link href="/pricing" className="text-center font-body text-sm font-semibold text-gold underline-offset-4 hover:underline">
            Full pricing
          </Link>
          <Link href="/#features" className="text-center font-body text-sm font-semibold text-gold underline-offset-4 hover:underline">
            Platform features
          </Link>
        </div>
      </nav>
