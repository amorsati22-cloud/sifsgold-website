...
import { PHOTO_STUDIO_NAV } from "@/lib/photo-studio/constants";

export function PhotoStudioShell({
  children,
  title,
  description,
}: {
  ...
  return (
    <div className="relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2 bg-navy text-cream">
      ...
      <div className="mx-auto flex max-w-content flex-col gap-8 px-4 py-8 lg:flex-row lg:px-8">
        <nav className="lg:w-52 lg:shrink-0" aria-label="Photo Studio">
          <ul className="flex list-none flex-wrap gap-2 p-0 lg:flex-col lg:gap-1">
            {PHOTO_STUDIO_NAV.map((item) => {
              const active =
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`block rounded-brand-md px-3 py-2 font-body text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-navy ${
                      active
