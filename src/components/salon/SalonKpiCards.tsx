...
export function SalonKpiCards({ kpis }: { kpis: Kpi[] }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {kpis.map((k) => (
        <div
          key={k.label}
          className="rounded-brand-lg border border-gold/15 bg-navy/40 px-4 py-5"
        >
          ...
          {k.hint ? <p className="mt-1 font-body text-xs text-cream/70">{k.hint}</p> : null}
        </div>
      ))}
    </div>
  );
